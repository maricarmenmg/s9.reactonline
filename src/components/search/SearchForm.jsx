import { useState } from 'react';

const SearchForm = ({ onSearch }) => {

  const [query, setQuery] = useState('');


  const handleChange = (event) => {
    setQuery(event.target.value);
  };

  const handleSubmit = (event) => {
    event.preventDefault();
    onSearch(query);
  };

  return (

    <div className="">
      <form onSubmit={handleSubmit}>
        <div className="flex">
          <input
            type="search"
            value={query}
            onChange={handleChange}
            placeholder="Buscar películas o programas de televisión"
            className="border-gray-300 focus:border-zinc-500 focus:ring focus:ring-zinc-200 focus:ring-opacity-50 rounded-full flex-1 block w-full sm:text-sm border px-4 py-2"
               
          />
 
        </div>
      </form>

    <button type="button" className="hidden h-8 w-full items-center gap-2 rounded-full bg-white pl-2 pr-3 text-sm text-zinc-500 ring-1 ring-zinc-900/10 transition hover:ring-zinc-900/20 dark:bg-white/5 dark:text-zinc-400 dark:ring-inset dark:ring-white/10 dark:hover:ring-white/20 lg:flex focus:[&amp;:not(:focus-visible)]:outline-none">
    <svg viewBox="0 0 20 20" fill="none" aria-hidden="true" class="w-5 h-5 stroke-current">
    <path stroke-linecap="round" stroke-linejoin="round" d="M12.01 12a4.25 4.25 0 1 0-6.02-6 4.25 4.25 0 0 0 6.02 6Zm0 0 3.24 3.25">
    </path>
    </svg>Find something...
    </button>

    </div>
  );
};

export default SearchForm;
